...
hero = hero.replace(
  /<div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">\s*<img src=\{img\} alt="" className="w-full h-full object-cover blur-xl opacity-30 scale-110" \/>\s*<\/div>\s*<img \s*src=\{img\} \s*alt=\{\`Slide \$\{index\}\`\} \s*className="relative z-10 w-full h-full object-contain object-center"\s*fetchpriority=\{index === 0 \? "high" : "auto"\}\s*\/>/g,
  `<img 
              src={img} 
              alt={\`Slide \${index}\`} 
...
console.log('Fixed hero layout again');
