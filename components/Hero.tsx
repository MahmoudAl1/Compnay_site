...
export const Hero: React.FC<HeroProps> = ({ onAction, lang, translations }) => {
  ...
  return (
    <div className="relative bg-slate-950 overflow-hidden flex flex-col">
      {/* Slider Section */}
      <div className="relative w-full bg-slate-900 overflow-hidden">        {isLoadingImages && heroImages.length === 0 && (          <div className="w-full aspect-[2/1] md:aspect-[3/1] lg:aspect-[1920/650] flex items-center justify-center bg-slate-800 relative">            <div className="animate-pulse w-full h-full bg-slate-800 absolute inset-0"></div>            <div className="absolute flex flex-col items-center z-10">              <div className="w-10 h-10 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>            </div>          </div>        )}        {heroImages.map((img, index) => (          <div             key={index}            className={`${index === 0 ? "relative" : "absolute inset-0"} transition-opacity duration-1000 ease-in-out w-full h-full flex items-center justify-center ${              index === currentSlide ? "opacity-100 z-10" : "opacity-0 z-0"            }`}          >            <img               src={img}               alt={`Slide ${index}`}               className="w-full h-auto block"              fetchpriority={index === 0 ? "high" : "auto"}            />          </div>        ))}
        {/* Slide Indicators */}
